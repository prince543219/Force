...
async def check_join_and_unmute(client, user_id, user_obj=None):
    for _ in range(10):  # 10 retries = ~20s
        if await is_user_in_channel(client, user_id):
            ...
            await client.send_message(GROUP_ID, f"✅ {mention} joined the channel and is now unmuted!")
            return
        await asyncio.sleep(2)

# Monitor messages from users not in the channel
